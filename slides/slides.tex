    ...
    \begin{column}{.5\linewidth}
      ...
      \only<2->{
        \resizebox{\linewidth}{!}{
          \begin{tikzpicture}
            \pie[
              radius=2.2, explode=.2, % text=pin,
              color={hsr-blue40, hsr-mauve40, hsr-lakegreen40, hsr-petrol40, hsr-reed40}
            ]{
              12.9/No clue,
              35.5/Have heard of Git,
              22.6/Noob wizards,
              25.8/Struggling,
               3.2/Pretty good
            }
...
%   \begin{tikzpicture}
%     \only<1>{
%       \node[font=\Huge\bfseries, text width = .8\linewidth]{Work together?\newline Open Source? \newline Literally communism};
%     }
%     \only<2-4>{
...
% 
%       \node[fill=white, font=\large\bfseries] at (s.north) {Richard Stallman};
% 
%       \node[fill=white, font=\large\bfseries, text width=5cm] (t) at (-3, 0){
%         Almost all companies and research institutes use a Cloud service for
%         data and / or Git (some stats put Git at > 90\% market share for VCS).
%         \\ \hspace{1cm} \\
%         So, no, Git and open source are not communism. Git is widely used in
%         industry.
%       };
%     }
%     \only<5-8>{
...
% 
%       \node[font=\bfseries\large, fill=white, below, align=center, text width = .8\linewidth] at (t.south)
%         {Political discourse ($\neq$ political progress) desperately needs to slow down.};
%     }
%     \only<9->{
    ...
    \begin{column}{.5\linewidth}
      \begin{tikzpicture}
        ...
        \only<2->{
          ...
          \begin{scope}[font=\ttfamily]
            \node[below = 1mm] at (n-1-3) {BX};
            \node[above = 1mm] at (n-1-5) {BY};
          \end{scope}

          \draw[very thick, ->] (m) -- node[midway, right]
            {in \texttt{BY} rebase \texttt{BX}} (n);
        }
      \end{tikzpicture}
    \end{column}
    \begin{column}{.5\linewidth}
  ...
  \only<2->{
    \begin{columns}
      ...
      \begin{column}{.5\linewidth}
        \begin{block}{Plan}<3->
          \begin{enumerate}
            \item Short introduction of tutorial
