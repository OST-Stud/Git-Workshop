...
\begin{frame}[fragile]{Auomatic Merge Failed (Conflicts)}
  \begin{columns}
    \begin{column}{.95\paperwidth}
      \centering
      \begin{tikzpicture}[]
  ...
  \begin{columns}
    ...
    \begin{column}{.5\linewidth}
      \begin{alertblock}{Logarithmic Search}
        Start with $Q = S$ then
        \begin{enumerate}
          \item take $J \in Q$ in the ``middle'' of $Q$ and if $q \in Q$ we are done
          \item otherwise
            \begin{enumerate}
...
\begin{frame}[fragile]{Mathematical Digression: Logarithmic Search III}
